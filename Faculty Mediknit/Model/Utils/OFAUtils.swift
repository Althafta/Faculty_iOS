...
//        return emailTest.evaluate(with: stringEmail)
    ...
    class func showLoadingViewWithTitle(_ title:String?){
        ...
        loadingView.spinnerColor=self.getColorFromHexString(barTintColor).withAlphaComponent(0.5)//UIColor.lightGray
        loadingView.titleTextColor = self.getColorFromHexString(barTintColor)//UIColor.white
        loadingView.spinnerLineWidth=2.0
        loadingView.foregroundColor=self.getColorFromHexString("F8FAFF")
...
//        formatter.dateStyle = .none
    ...
    class func getSurgeryDate(_ stringDate:String)->Date{
        let dateFormatter = DateFormatter()
    ...
    class func getStringTimeFromDate(_ date:Date)->String {
        ...
        let local = Locale(identifier: "da_DK")
        formatter.locale=local
        return formatter.string(from: date)
    }
